...
void  make_KEK_data_histos_column(TString input_path, TString file_name, TString output_path){
  ...
  for(int j=0;j<16;j++){
	...
	h[j][jj][jjj] = new TH1D(Form("time_slot_%i_col_%i_row_%i",j+1,jj+1,jjj+1),Form("time_slot_%i_col_%i_row_%i",j+1,jj+1,jjj+1),n_bins,low_time,up_time);
...
Fit_results_basic basic_fit_data(TString input_basepath,
  ...
  */
  ...
  if(fit_model_r==4){
    ...
    Results.frac_H_KR[0]=frac_KR_C_0.getVal();
    Results.frac_H_KR[1]=frac_KR_C_0.getError();
  }else{
...
void loop_fit_column(TString input_basepath, TString input_basefilename, TString output_basepath, int slot_ID, int column_number,int fit_model_ID){
  /*
    To Run:
  ...
  */
  ...
  for(int h=1;h<=8;h++){
    ...
    my_Results = basic_fit_data("",input_basefilename,2,fit_model_ID,slot_ID,3,column_number,h);   //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    cc->cd(h);
